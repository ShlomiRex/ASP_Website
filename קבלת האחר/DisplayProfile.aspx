...
<script runat="server">
    ...
    void Page_Load()
    {
        ...
        else if (Session["isUserRegistered"] == null || (bool)Session["isUserRegistered"] == false) 
            Response.Redirect("LogIn.aspx");
        else Response.Write("Home.aspx");
    }
</script>
...
<% Session["alert"] = null;%>
    ...
    <%
        ...
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            ...
            if((bool)ds.Tables[0].Rows[i]["isAdmin"] == false)
            {
            htmlString += "<td> <a name='DeleteUserButton' href='DeleteUser.aspx?textBox="+ds.Tables[0].Rows[i]["userName"].ToString()+"'>Delete</a> </td>";
            htmlString += "<td> <a name='UpdateUserButton' href='UpdateUser.aspx?nameOfUser="+ds.Tables[0].Rows[i]["userName"].ToString()+"'>Update</a></td>";
            }
            htmlString += "</tr>";
        }
    %>
